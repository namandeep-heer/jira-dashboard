...
const DEV_NOT_FINISHED_PHASES = new Set(['Product', 'Developer Pending', 'Developing', 'On Hold']);
const DEV_NOT_FINISHED_STATUS_RES = [
  /(creating|^prod)/i,
  /^dev[\s-]*(pending|developing|designing|grooming|cr|merge|reopened)/i,
  /^on[\s-]*hold/i,
];

function isDevNotFinished(ticket) {
  const phase = ticketPhase(ticket);
  if (DEV_NOT_FINISHED_PHASES.has(phase)) return true;
  const status = String(ticket && ticket.status || '').trim();
  return !!status && DEV_NOT_FINISHED_STATUS_RES.some(re => re.test(status));
}
...
function renderInsightGroup(list) {
...
function renderInsightsPanel(tickets) {
  const list = tickets || [];
  const withDev = list.filter(isDevNotFinished);
  const afterDev = list.filter(t => !isDevNotFinished(t));
  return `
    <div class="kpi-grid">
      <div class="kpi"><div class="v">${list.length}</div><div class="l">All tickets</div></div>
      <div class="kpi"><div class="v">${withDev.length}</div><div class="l">Still with development</div></div>
      <div class="kpi"><div class="v">${afterDev.length}</div><div class="l">After development</div></div>
    </div>
    <div class="insight-split">
      <section class="insight-section">
        <div class="insight-section-head">
          <h2>Still with development</h2>
          <p>Product, developer pending, developing, and on hold — development is not finished yet.</p>
        </div>
        ${renderInsightGroup(withDev)}
      </section>
      <section class="insight-section">
        <div class="insight-section-head">
          <h2>After development</h2>
          <p>QA, rejected / replied, closed, and every later status.</p>
        </div>
        ${renderInsightGroup(afterDev)}
      </section>
    </div>`;
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .insight-split { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; align-items: start; }
  @media (max-width: 960px) { .insight-split { grid-template-columns: 1fr; } }
  .insight-bars { display: flex; flex-direction: column; gap: 7px; }
  .insight-bar-row { display: grid; grid-template-columns: 120px 1fr 36px; gap: 8px; align-items: center; font-size: 12px; }
  ...
  .insight-section-head { margin: 8px 0 14px; }
  .insight-section-head h2 { margin: 0 0 4px; font-size: 20px; }
  .insight-section-head p { margin: 0; color: var(--muted); font-size: 13px; }
  .toc {
