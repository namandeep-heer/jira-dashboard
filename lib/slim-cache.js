 ...
 * Config-page keys are then split into config/*.json by lib/page-config.js.
...
function slimDashboardState(state) {
  ...
  const releaseCache = state.releaseCache && typeof state.releaseCache === 'object'
    ? { ...state.releaseCache }
    : {};
  const selectedReleases = Array.isArray(state.activeReleases) ? state.activeReleases.filter(Boolean) : [];
  const mergedView = selectedReleases.length > 1;
  if (!mergedView && state.cache && typeof state.cache === 'object' && Object.keys(state.cache).length) {
...
function dashboardStateForClient(state) {
  ...
  const selected = Array.isArray(slimmed.activeReleases) ? slimmed.activeReleases.filter(Boolean) : [];
  if (selected.length > 1 && slimmed.releaseCache) {
    const merged = {};
    selected.forEach(name => {
      const bucket = slimmed.releaseCache[name];
      if (!bucket || typeof bucket !== 'object') return;
      Object.keys(bucket).forEach(projKey => {
        const src = bucket[projKey];
        if (!src) return;
        if (!merged[projKey]) {
          merged[projKey] = { ...src, issues: [...(src.issues || [])] };
          return;
        }
        const seen = new Set((merged[projKey].issues || []).map(issue => issue && issue.key));
        (src.issues || []).forEach(issue => {
          if (!issue || !issue.key || seen.has(issue.key)) return;
          seen.add(issue.key);
          merged[projKey].issues.push(issue);
        });
        merged[projKey].total = (merged[projKey].issues || []).length;
      });
    });
    slimmed.cache = merged;
  } else {
